...
### Page inventory (19 pages, one Firestore-backed app each)
`index.html` (dashboard) · `agenda.html` (read-only calendar of upcoming activities, sectorized by fazenda) · `mapa.html` (talhão map) · `adubacoes.html` · `pulverizacoes.html` · `drench.html` · `irrigacao.html` (irrigation management — Leaflet map reusing registered talhão polygons as background context; admins draw individualized clickable **setor** polygons, the irrigador clicks a setor and logs a watering shift with start/end time (date defaults to today); setores are colored by recency-since-last-watering (heatmap: today→blue, 1-2d→green, 3-4d→yellow, 5-6d→orange, 7+→red, never→grey). Each setor belongs to a **talhão** (required at creation; fazenda is derived from the talhão), and the sidebar is a **Fazenda → Talhão (accordion) → Setores** hierarchy so the field worker finds the talhão first, then picks the setor inside it; talhão rows sort most-overdue first and opening one zooms the map to it. Per-setor history + full-history modal. Admins can bulk-**import setores from Google Earth KML/KMZ** (JSZip via CDN unzips KMZ; DOMParser reads Placemark polygons — KML is lng,lat), each imported polygon mapped to a talhão (auto-suggested by centroid point-in-polygon). Collections: `irrigacaoSetores` (`{nome,fazenda,talhaoId,talhaoNome,coordenadas,...}`), `irrigacaoRegistros`) · `atividades.html` (legacy, not in active use — see note below) · `equipe.html` · `estoque.html` · `pedidos.html` · `cotacoes.html` (supplier price quotes — cloned from `pedidos.html`'s map/area-selection; the Planejamento tab plans what's needed, the Cotação tab collects per-supplier prices — including per-supplier alternative products — and compares, the Comparar tab derives products+prices from the cotações and lets an agronomist register each product's guarantees/composition to rank cost per kg of active ingredient. Collections: `cotacoes`, `garantiasProdutos`) · `gestao.html` (cross-module reporting) · `analises.html` · `recomendacoes.html` · `planejamento.html` · `admin.html` · `login.html` · `design-preview.html`.
